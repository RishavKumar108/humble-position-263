...
// scanner problem
	public void UpdateExistingCustomer() throws  CustomerException {
		...
		System.out.println(consolecolor.YELLOW+"enter Customer amount"+consolecolor.RESET);
		int Amount = sc.nextInt();
		int p = dao.UpdateExistingCustomer(name, email, Password, AccountNo, Amount, id);
		if (p == 1) {
			System.out.println(
					consolecolor.BLACK_BACKGROUND + consolecolor.WHITE + "Update your data" + consolecolor.RESET);
		} else {
			System.out.println(consolecolor.RED+"worng Information"+consolecolor.RESET);
		}
